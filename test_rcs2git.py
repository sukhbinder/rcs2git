...
class TestRcs2Git(unittest.TestCase):
    def test_parse_rcs_date(self):
        # Standard RCS date format
        dt = datetime.datetime(2023, 10, 27, 10, 30, 0, tzinfo=datetime.timezone.utc)
        self.assertEqual(parse_rcs_date("2023.10.27.10.30.00"), int(dt.timestamp()))

        # 2-digit year (should be treated as 19xx)
        dt = datetime.datetime(1999, 12, 31, 23, 59, 59, tzinfo=datetime.timezone.utc)
        self.assertEqual(parse_rcs_date("99.12.31.23.59.59"), int(dt.timestamp()))

        # ISO format fallback
        dt = datetime.datetime(2023, 10, 27, 10, 30, 0, tzinfo=datetime.timezone.utc)
        self.assertEqual(parse_rcs_date("2023-10-27T10:30:00Z"), int(dt.timestamp()))

        # Invalid format should return current time (approximately)
        now = int(time.time())
        self.assertAlmostEqual(parse_rcs_date("invalid date"), now, delta=2)
...
class TestIntegration(unittest.TestCase):
    def test_rcs2git_integration(self):
        # Mock rlog command
        rlog_output = """
revision 1.2
...
"""
        ...
        with patch('subprocess.check_output', side_effect=mock_subprocess) as mock_check_output:
            ...
            self.assertIn("blob", output)
            self.assertIn("commit refs/heads/master", output)
            self.assertIn("author testuser", output)
            self.assertIn("Initial revision", output)
            self.assertIn("Second revision", output)
